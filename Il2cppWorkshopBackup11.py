...
def dectohex(dec,bigendian = False,sep = " ",upper = True):
    if bigendian:
        raise Exception("Bigendian dec to hex not currently supported - oops!")
    ...
    #Pad Bytes With 0
    newbytes = []
    for thisbyte in byteslist:
        if len(thisbyte) == 1:
            newbytes.append("0" + thisbyte)
        else:
            newbytes.append(thisbyte)
...
def read_file(path,readtype = "r",hexmode = True,upperhex = True):
...
def armtohex(armcode,armtype,sep = " ",upper = True):
    ...
    for i, instruction in enumerate(armcode):
        ...
        try:
            convertedbytes.append(ks.asm(instruction,as_bytes = True)[0])
        except keystone.KsError as exception:
            print(armcode)
            print(exception)
            armhexconversionerror(armcode,i + 1,instruction,armtype,exception)
            return(None)
    sys.exit(str(convertedbytes))
    #Convert bytes to hex
    convertedhex = str(binascii.hexlify(convertedbytes,sep,1))
    #Remove b' prefix
    convertedhex = removebprefix(convertedhex)
    #Convert hex to list of bytes
    byteslist = getbytes(convertedhex)
    #Convert bytes back to hex with sep
    newhex = bytestohex(byteslist,sep)
    if upper:
        #Convert to uppercase
...
def hextoarm(hexstr,armtype,returntype = "list"):
    ...
    for hexinstruction in hexinstructions:
        ...
        try:
            ...
            print(hexstr)
            print(exception)
            hexarmconversionerror(hexstr,armtype,exception)
            return(None)
