...
def dectohex(dec,bigendian = False,sep = " ",upper = True):
    ...
    newhex = bytestohex(newbytes,sep)
        ...
        try:
            ...
            if content is None:
                ...
                if ("b" in readtype) and hexmode:
                    try:
                        formathex(binascii.hexlify(content,hexsep,upperhex,True))
                    except Exception:
...
def formathex(hexstring,sep = " ",upper = True):
    if type(hexstring) == bytes:
        ...
        newhex = removebprefix(hexstring)
    #Convert hex to list of bytes
    byteslist = getbytes(newhex)
    #Convert bytes back to hex with sep
...
def getfullobjects(dumpcs,getshared = True,toremoveattributes = True,toremoveblanklines = True,toremoveallblanklines = False,returntuple = True):
    #Dumpcs will automatically become global dumpcs if local dumpcs is None
...
def getpropetiesdict(propeties):
    fieldsdict = {}
    for thisproperty in propeties:
        propetiesdict[thisproperty["Name"]] = thispropety
    return(propetiesdict)
...
def builddumpcshierarchy(dumpcspath,doalphabeticalsort = True,onlyclasses = False,getshared = True,namespacefilter = None,toremoveattributes = True,toremoveblanklines = True,toremoveallblanklines = False,rettype = "dict"):
    ...
    if dumpcs is None:
        return(None)
    fullobjects = getfullobjects(dumpcspath,getshared,toremoveattributes,toremoveblanklines,toremoveallblanklines,returntuple = True)
    if doalphabeticalsort:
...
def comparativedeobfuscationdemo(classestofind):
    ...
    startspeedtest()
    fullobjects = getfullobjects(dumpcs,True)
    endspeedtest()
...
def deobfuscateallclassesdemo():
   ...
   loaddumpcs(dumpcspath)
   startspeedtest()
   fullobjects = getfullobjects(dumpcs,False)
   endspeedtest()
   print("All classes/structs/interfaces/enums extracted from dump.cs in " + timetaken + " miliseconds.")
   startspeedtest()
   fullclasses = getfullclasses(fullobjects)
...
init()
print(getobjects(getfullobjects(r"C:/Users/zachy/OneDrive/Documents/Work/Projects/Pixel Gun 3D/Pixel Gun 3D 16.6.1/Pixel Gun 3D 16.6.1 dump.cs")))
sys.exit()
