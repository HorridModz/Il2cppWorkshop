...
#If you have moved Il2cppWorkshop.py to a new directory, please change it to the correct path.
...
# - Auto-cheat Disarmer: Can automatically generate functions to encrypt and decrypt obscured types, disable certain anti-cheats like codestage, etc. This method relies on anticheats having the same class, method, and field names. It can also attempt to deobfuscate using the known anti-cheat classes as the unobfuscated, with methods such as brute force deobfuscation and aob deobfuscation (this works best if the game has obfuscated names)
            ...
            def addtypeprefixesandsuffixes(prefixes,suffixes):
                ...
                for thisprefix in prefixes: #prefixes and suffixes
                    for thistype in _types: #use the new value types that already have suffixes, but not prefixes
                        types.append(thisprefix + thistype)
                _types = newtypes #set valuetypes to the newtypes now that we are done iterating through it
                #Prefixes
...
##            thisbyte = ""
    byteslist = wraptext(hexstring,2)
    return(byteslist)
...
class ModMenuInstall():
    def __init__(self,apkpath,newapkpath,modmenuapkpath,menutype = "LGL",modmenulibname = "libMyLibName"):
        ...
        for thissmali in allsmali:
            ...
            for thismodmenusmalifolder in modmenusmalifolders:
                if thismodmenusmalifolder in thissmalifolders:
                    conflict = True
            if not(conflict):
                foundsmali = thissmali
                break
        if foundsmali is None:
            #Failed: No smali folder found without naming conflict
            return(None)
        copyfolder(modmenusmali,foundsmali,cancelifoverwrite = False)
    def modifylib(self):
    ...
    for thisoffset, thisbyte in enumerate(fullhexbytes):
        ...
        if (currentmatchbyte == unknownbyte or thisbyte == currentmatchbyte):
            ...
            if inmatch:
                ...
                #print(currentmatchindex)
                if currentmatchindex == len(aobbytes) - 1:
                    print(currentmatch)
                    print(currentmatchindex)
...
def main():
    ...
    print(script)
    sys.exit()
    print(aobsearch(r"C:\Users\zachy\OneDrive\Documents\Work\Tools\Il2cppWorkshop\Il2cppWorkshop.cpp","?? 2F 20 53 69 6D 70 6C 65 20 49 6C ?? ?? 70 70 57 6F 72 6B 73 68 6F 70 20 4C 61 ?? 6E 63 68 65 72 0D 0A 2F 2F 20 4D 61 6B 65 ?? 73 75 72 65 20 74 68"))
    ...
    menu = ModMenuInstall(apk,new,modmenupath,"LGL","libModMenu.so")
